...
const MOCK_FIT_SCORE: FitScore = {
  ...
  categories: [
    ...
    },
    {
      ...
      score: 75,
      reason: "Relevant project experience",
    },
    {
      name: "Education Background",
      score: 90,
      reason: "Excellent academic background",
    },
  ],
};
...
const MOCK_STUDY_PLAN: StudyDay[] = [
  ...
  },
  {
    ...
    description:
      "Learn database optimization and horizontal scaling strategies",
    leetcodeQuestions: [
...
const MOCK_PRACTICE_QUESTIONS: PracticeQuestion[] = [
  ...
  },
  {
    ...
    description:
      "Discuss strategies for horizontal partitioning of database data.",
    keyDiscussionPoints: [
      "Shard key selection",
      "Consistent hashing",
      "Cross-shard queries",
...
const MOCK_PEOPLE_IN_ROLE: Person[] = [
  ...
  },
  {
    ...
    yearsInRole: 5,
    background:
      "Transitioned from IC to management, responsible for team hiring",
    interviewTip:
 ...
 */
function mapBackendDataToInsights(
  backendData: BackendPipelineResponse
): InsightsData {
  ...
  if (profile.experience && profile.experience.length > 0) {
    ...
    if (firstExp.start_date) {
      const startYear = parseInt(
        firstExp.start_date.split(" ")[1] || new Date().getFullYear().toString()
      );
  ...
  const technicalSpecialties =
    profile.experience
      ...
      .filter(Boolean)
      .join(", ") ||
    profile.user_info?.headline ||
    "Not specified";

  // Use job skills for predicted question areas
  const predictedQuestionAreas =
    topics.slice(0, 4).join(", ") || "General technical questions";

  // Build background summary
  const backgroundSummary =
    profile.user_info?.headline ||
    `${profile.user_info?.name || "Interviewer"}${
  ...
  const recommendedImprovements =
    allReasons.join(". ") || "Continue building on your strengths";
  ...
  const categories: FitScoreCategory[] = Object.entries(fitScoreCategories).map(
  ...
  const companyName =
    backendData.company_name
      ?.replace("The company name is ", "")
      .replace(".", "") ||
    jobData?.job_info?.company ||
    "Company";

  const companyValues =
    jobData?.description?.summary ||
    `${companyName} is looking for talented individuals to join their team.`;

  const hiringStyle =
    jobData?.description?.requirements?.must_have?.slice(0, 3).join(", ") ||
    jobData?.description?.requirements?.nice_to_have?.slice(0, 3).join(", ") ||
    "Not specified";

  const cultureSummary =
    jobData?.description?.summary ||
    `Join ${companyName} for an exciting opportunity in ${
      jobData?.job_info?.location || "their team"
    }.`;

  const interviewPatterns =
    `Position: ${jobData?.job_info?.title || "Software Engineer"} | ` +
  ...
  const peopleInRole: Person[] = (backendData.references?.references || []).map(
    (ref) => ({
      name: ref.name,
      role: "Current Employee",
      yearsInRole: 0, // Not available in backend response
      background: ref.linkedin_url
        ? `LinkedIn Profile: ${ref.linkedin_url}`
        : "Employee at company",
      interviewTip:
        "Reach out to learn more about the interview process and company culture",
      waysToConnect: [
        ref.linkedin_url || "",
...
export default function InsightsPage() {
  ...
  const [companyResearch, setCompanyResearch] =
    useState<CompanyResearch | null>(MOCK_COMPANY_RESEARCH);
  const [studyPlan, setStudyPlan] = useState<StudyDay[]>(MOCK_STUDY_PLAN);
  const [practiceQuestions, setPracticeQuestions] = useState<
    PracticeQuestion[]
  >(MOCK_PRACTICE_QUESTIONS);
  const [peopleInRole, setPeopleInRole] =
    useState<Person[]>(MOCK_PEOPLE_IN_ROLE);
              ...
              <div className="relative bg-white/10 backdrop-blur-xl rounded-lg shadow-lg border border-white/20 p-6">
                ...
                <div className="relative">
                  ...
                  {(() => {
                    ...
                    if (storedData) {
                      try {
                        const data: BackendPipelineResponse =
                        ...
                        return (
                          <div className="space-y-4">
                            ...
                            {/* Requirements Grid */}
                            {(description?.requirements?.must_have?.length >
                              0 ||
                              description?.requirements?.nice_to_have?.length >
                                0) && (
                              <div className="border-b border-gray-200 pb-4">
                                    ...
                                    description.requirements.must_have.length >
                                      ...
                                      <div>
                                        ...
                                        <ul className="space-y-1.5">
                                          {description.requirements.must_have.map(
                                            (req, idx) => (
                                              <li
                                                key={idx}
                                  ...
                                  {description.requirements.nice_to_have &&
                                    description.requirements.nice_to_have
                                      ...
                                      <div>
                                        ...
                                        <ul className="space-y-1.5">
                                          {description.requirements.nice_to_have.map(
                                            (req, idx) => (
                                              <li
                                                key={idx}
                                                className="text-xs text-black flex items-start gap-2"
                              ...
                              <div className="border-b border-gray-200 pb-4">
                                ...
                                <div className="space-y-3">
                                  {description.skills.technical &&
                                    description.skills.technical.length > 0 && (
                                      <div>
                                        <p className="text-xs text-gray-600 mb-2">
                                          Technical
                                        </p>
                                        <div className="flex flex-wrap gap-2">
                                          {description.skills.technical.map(
                                            (skill, idx) => (
                                              <span
                                                key={idx}
                                  ...
                                  {description.skills.soft &&
                                    description.skills.soft.length > 0 && (
                                      <div>
                                        <p className="text-xs text-gray-600 mb-2">
                                          Soft Skills
                                        </p>
                                        <div className="flex flex-wrap gap-2">
                                          {description.skills.soft.map(
                                            (skill, idx) => (
                                              <span
                                                key={idx}
                                  ...
                                  {description.skills.tools_and_technologies &&
                                    description.skills.tools_and_technologies
                                      .length > 0 && (
                                      <div>
                                        <p className="text-xs text-gray-600 mb-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                          {description.skills.tools_and_technologies.map(
                                            (tool, idx) => (
                                              <span
                                                key={idx}
                                  ...
                                  {description.compensation_and_benefits
                                    ...
                                    description.compensation_and_benefits
                                      ...
                                      <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                          {description.compensation_and_benefits.benefits.map(
                                            (benefit, idx) => (
                                              <span
                                                key={idx}
                  ...
                  <div className="relative">
                    ...
                    {interviewerIntel ? (
                      <div className="space-y-4">
                        ...
                        {interviewerIntel.affiliations &&
                          interviewerIntel.affiliations.length > 0 && (
                            <div className="border-b border-gray-200 pb-3">
                              <p className="text-sm text-gray-600 mb-2">
                                Affiliations
                              </p>
                              <div className="flex flex-wrap gap-2">
                                {interviewerIntel.affiliations.map(
                                  (affiliation, idx) => (
                                    <span
                                      key={idx}
                        ...
                        <div className="relative w-32 h-32 mb-6">
                          ...
                          >
                            ...
                            <circle
                              ...
                              strokeDashoffset={`${
                                2 * Math.PI * 56 * (1 - fitScore.score / 100)
                              }`}
                              strokeLinecap="round"
                            />
                          </svg>
          ...
          <div className="space-y-6">
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Complete daily topics with tagged LeetCode questions and
                resources
              </p>
            </div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    },
                    {
                      ...
                      duration: "60 minutes",
                      details:
                        "Scalable system design for real-world scenarios",
                    ...
                    },
                    {
                      ...
                      duration: "30 minutes",
                      details:
                        "Leadership, teamwork, and problem-solving stories",
                      ...
                      <div className="ml-16 space-y-4">
                        ...
                        <div>
                          <h3 className="text-base font-semibold text-gray-800 mb-3">
                            LeetCode Tagged Questions (
                            {day.leetcodeQuestions.length})
                          </h3>
        ...
        {activeTab === "Mock Interview" && (
          <div className="space-y-6">
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Practice with AI-powered mock interviews to prepare for your
                upcoming interview
              </p>
            </div>
            ...
            <AnimatedScrollSection>
              <div className="relative bg-white/10 backdrop-blur-xl rounded-lg shadow-lg border border-white/20 p-8 text-center z-50">
                <GlowingEffect
                  variant="red"
                  spread={40}
                  glow={true}
                ...
                <div className="relative z-60">
                  ...
                  <p className="text-gray-600 mb-6">
                    Practice answering interview questions with AI-powered
                    feedback
                  </p>
                  <button
                    onClick={(e) => {
                    ...
                    className="relative z-60 bg-red-700 text-white text-lg font-bold px-8 py-4 rounded-md hover:opacity-90 transition-opacity shadow-lg hover:cursor-pointer"
        ...
        {activeTab === "People in Role" && (
          <div className="space-y-6">
            ...
            <div className="mb-6 text-center">
              ...
              <p className="text-gray-600">
                Connect with current employees and learn from their interview
                experiences.
              </p>
            </div>
                      ...
                      <div className="border-t border-gray-200 pt-4 mb-4">
                        ...
                        <p className="text-sm text-gray-600">
                          {person.background.includes("LinkedIn Profile:")
                            ...
                            : person.background ||
                              "Professional connection at the company"}
                        </p>
                      </div>
